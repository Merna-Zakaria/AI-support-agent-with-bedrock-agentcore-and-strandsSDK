...
1. In the AWS Lambda console, create two new functions (Python 3.12 runtime):
   - `order-tracker`------------------> arn:aws:lambda:us-east-1:627764425123:function:order-tracker
   - `refund-processor`---------------> arn:aws:lambda:us-east-1:627764425123:function:refund-processor
...
AgentCore Gateway uses these operation names when exposing the routes as MCP tools.

Open API Gateway → Create API → choose REST API → Build.
pick a name like order-tracker-api
create the API
Create the first resource path:
in Resources, click Create resource
create /orders
with /orders selected, click Create resource again
enable Configure as proxy resource only if the UI forces that pattern; otherwise create child resource {order_id}
final path should be /orders/{order_id}
Add the GET method to /orders/{order_id}:
select /orders/{order_id}
click Create method → choose GET
integration type: Lambda function
turn on Lambda proxy integration
choose region us-east-1
select your order-tracker Lambda
save and allow API Gateway permission if prompted
Set the operation name:
with GET selected, open Method request or Method settings area
find Operation name
set it to get_order
Create the second route:
create resource /customers
under it create child resource {customer_id}
under {customer_id} create child resource orders
final path should be /customers/{customer_id}/orders
Add GET for /customers/{customer_id}/orders:
same integration settings:
Lambda function
Lambda proxy integration
target = order-tracker
set operation name to get_customer_orders
Add the third route:
select /customers/{customer_id}
create GET method
same Lambda proxy integration to order-tracker
set operation name to get_customer
Deploy the API:
click Deploy API
...
printf "REST_API_ID=\nSTAGE_NAME=prod\n" > api_gateway_notes.txt
Test the routes in API Gateway or from the browser/terminal once deployed. The URL shape will be like:
bash

https://REST_API_ID.execute-api.us-east-1.amazonaws.com/prod/orders/123
One important caution: don’t invent the exact test IDs unless the Lambda code

--------------------------------------------------------------------------------------

### Step 1.3 — Set Up the AgentCore Gateway

The Gateway exposes the API Gateway operations and refund Lambda tools through the Model Context Protocol (MCP).

Open Amazon Bedrock → AgentCore → Gateways.

Create a Gateway named CustomerSupportGateway.

Select the NONE authorizer.

The provided starter code connects to the Gateway without a bearer token or AWS request signing. Using another authorizer would require additional authentication code.

Add the order-tracking API as an API Gateway target:

Setting	Value
Target name	order-tracker
Target type	API Gateway REST API stage
REST API	The REST API created in Step 4
Stage	prod, or the stage name you selected
Operations	The three GET operations created in Step 4
Add the refund function as a separate Lambda target:

Setting	Value
Target name	refund-processor
Target type	Lambda function
Lambda function	refund-processor
Tool schema	starter/lambda/lambda_schema
Copy the Gateway URL, which ends with /mcp. Add it to main.py: GATEWAY_URL = "https://<alias>.gateway.bedrock-agentcore.<region>.amazonaws.com/mcp"  
